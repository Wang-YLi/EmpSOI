...
class EmpSOI(nn.Module):
    ...
    def forward(self, batch):
        ...
        emo_logits = self.emo_lin(torch.concat([emo_cls.squeeze(1),self_high_emo_state.squeeze(1),last_cls_embs],dim=1))
        ...
        graph_ref_ctx= self.graph_ref_fusion(torch.cat([enc_outputs, self_high_emo_state.unsqueeze(1).expand_as(enc_outputs)], dim=-1))
