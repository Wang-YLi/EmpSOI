...
from model.beam import Translator
...
def gen_embeddings(vocab):
    ...
    if config.emb_file is not None:
        ...
        pre_trained = 0
        for line in open(config.emb_file,encoding='mac_roman').readlines():
            sp = line.split()
...
def print_custum(emotion, dial, ref,hyp_g, pred_emotions):
    res = ""
...
def evaluate(model, data, ty="valid", max_dec_step=30):
    ...
    for j, batch in pbar:
        ...
        pbar.set_description(
            "loss:{:.4f} ppl:{:.1f}".format(np.mean(l), np.exp(np.mean(l)))
        )
    ...
    print("{}\t{:.4f}\t{:.4f}\t{:.4f}\n".format(ty, loss, np.exp(loss), acc))
    if ty == 'test':
        dist_1, dist_2 = calc_distinct(hyp_g)
        print("Dist-1\tDist-2")
        print("{:.4f}\t{:.4f}\n".format(dist_1, dist_2))

    return loss, np.exp(loss), bce, acc, results
