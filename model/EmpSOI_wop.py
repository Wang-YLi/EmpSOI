...
class EmpSOI(nn.Module):
    def __init__(
        ...
        self.emo_lin = nn.Linear(3*config.hidden_dim, emo_number,bias=False)
    ...
    def construct_graph(self, utt_hidden,utt_cls_index,conv_len ,x_attr,commonsense):
        ...
        other_perspective_taking = torch.zeros([utt_hidden.size(0), config.emb_dim]).unsqueeze(1).type_as(utt_hidden).cuda()
    ...
    def forward(self, batch):
        ...
        emo_logits = self.emo_lin(torch.concat([emo_cls.squeeze(1),other_high_emo_state.squeeze(1),last_cls_embs],dim=1))
        ...
        graph_ref_ctx= self.graph_ref_fusion(torch.cat([enc_outputs, other_high_emo_state.unsqueeze(1).expand_as(enc_outputs)], dim=-1))
        graph_ref_ctx,_ = self.graph_multihead(graph_ref_ctx, graph_ref_ctx, graph_ref_ctx,src_mask)
        
        enc_outputs = self.all_cross_attn(enc_outputs,cog_ref_ctx,graph_ref_ctx)+enc_outputs
