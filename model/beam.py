    ...
    def beam_search(self, src_seq, max_dec_step):
        ...
        def beam_decode_step(
            ...
            def predict_word(
                ...
                db_dist = None
                dim = [-1, dec_output.size(1), -1]
                dec_output = self.model.layer_norm(int_T.expand(dim)+dec_output)
        ...
        with torch.no_grad():
            ...
            for r in self.model.rels:
                emb = self.model.embedding(src_seq[r].long()).to(config.device)
                mask = src_seq[r].data.eq(config.PAD_idx).unsqueeze(1)
                cs_embs.append(emb)
                cs_masks.append(mask)
                enc_output = self.model.encoder(emb, mask)
                cs_outputs.append(enc_output)

            cls_tokens = [c[:, 0].unsqueeze(1) for c in cs_outputs]

            # Shape: batch_size * 1 * 300
            cog_cls = torch.tensor([item.cpu().detach().numpy() for item in cls_tokens[:-1]])
            emo_cls = cls_tokens[-1]
            cog_cls = cog_cls.view(cog_cls.size(1),5,config.hidden_dim)
            # Emotion
            emo_logits = self.model.emo_lin(torch.concat([src_enc[:, 0],emo_cls.squeeze(1)],dim=1))
            emo_T = self.model.emo_garrix(emo_logits)
            
            # Intent
            int_logits = self.model.int_lin(torch.concat([src_enc[:, 0],emo_cls.squeeze(1),emo_T],dim=1))
            int_T = self.model.int_garrix(int_logits) 
            int_T = int_T.reshape(int_T.size(0),1,int_T.size(1))
            # enc_outputs = self.layer_norm(int_T.expand(dim)+enc_outputs)
            # enc_outputs = self.layer_norm(int_T.expand(dim)+enc_outputs)
            # int_T = torch.cat([int_T,torch.zeros(int_T.size(0),enc_outputs.size(1)-1,config.hidden_dim).cuda()],dim=1).cuda()
            # enc_outputs = self.layer_norm(int_T+enc_outputs)

            # Cognition
            if (src_enc.size(1)-cog_cls.size(1))>=0:
                cog_cls = cog_cls.to(config.device)
                cog_logits = torch.cat([cog_cls,torch.zeros(cog_cls.size(0),src_enc.size(1)-cog_cls.size(1),config.hidden_dim).to(config.device)],dim=1)
                cog_logits = cog_logits.to(config.device)
                src_enc = self.model.layer_norm(cog_logits+src_enc)
            encoder_db = None
